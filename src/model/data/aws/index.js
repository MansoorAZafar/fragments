...
async function writeFragment(fragment) {
  logger.info({ fragment }, 'Writing Fragment Into DynamoDb');
  ...
  try {
    ...
    logger.info({ res }, 'writeFragment data is');
...
async function readFragment(ownerId, id) {
  logger.info({ ownerId, id }, 'readFragment() reading from this ownerId and id');
  ...
  try {
    ...
    logger.info({ data }, 'data read from dynamoDB');
...
async function deleteFragment(ownerId, id) {
  await deleteFromS3(ownerId, id);
  await deleteFromDynamoDB(ownerId, id);
}

async function deleteFromS3(ownerId, id) {
  const params = {
    Bucket: process.env.AWS_S3_BUCKET_NAME,
    // Our key will be a mix of the ownerID and fragment id, written as a path
    Key: `${ownerId}/${id}`,
...
async function deleteFromDynamoDB(ownerId, id) {
  const params = {
    TableName: process.env.AWS_DYNAMODB_TABLE_NAME,
    Key: { ownerId, id },
  };
